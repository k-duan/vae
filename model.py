...
class Encoder(nn.Module):
    def __init__(self, input_size: int, z_dim: int, dropout: float):
        super().__init__()
        # self._ffn = FFN(dims=[input_size, z_dim * 16, z_dim * 4], dropout=dropout)
        self._mu = nn.Linear(input_size, z_dim) # \mu
        self._log_sigma2 = nn.Linear(input_size, z_dim) # log(\simga^2)

    def forward(self, x: torch.Tensor):
        # x = self._ffn(x)
        return self._mu(x), torch.clamp(self._log_sigma2(x), min=-1.5, max=1.5)

class Decoder(nn.Module):
    def __init__(self, output_size: int, z_dim: int, dropout: float):
        super().__init__()
        # self._ffn = FFN(dims=[z_dim, z_dim * 4, z_dim * 16], dropout=dropout)
        self._mu = nn.Linear(z_dim, output_size)
        self._log_sigma2 = nn.Linear(z_dim, output_size)

    def forward(self, z: torch.Tensor):
        # z = self._ffn(z)
        return self._mu(z), torch.clamp(self._log_sigma2(z), min=-1.5, max=1.5)
...
class VAE(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = torch.randn_like(p_mu) * torch.exp(0.5 * p_log_sigma_2) + p_mu
        recon_loss, kl_loss, total_loss = self.compute_loss(x, p_mu, p_log_sigma_2, q_mu, q_log_sigma_2)
        return x.view(batch_size, 1, 28, 28), recon_loss, kl_loss, total_loss, p_mu, p_log_sigma_2, q_mu, q_log_sigma_2
